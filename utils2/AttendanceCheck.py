...
from utils2.GlobalVar import add_path_to_sys
rootdir = add_path_to_sys()

# 导入考勤状态判断相关函数和变量
from utils2.GlobalVar import COURSE_TIME, LATE_SPAN
...
def attendance_check(set_time='08:00:00'):
    ...
    print(f'{set_time}', '%Y-%m-%d %H:%M:%S')
    # 格式化考勤时间
    att_time = datetime.strptime(f'{set_time}', '%H:%M:%S')
    # 计算当前时间与设定时间的差值
